...
def benchmark(model, input_shape=(1024, 1, 32, 32), dtype='fp32', nwarmup=50, nruns=1000):
    ...
    with torch.no_grad():
        for i in range(1, nruns+1):
            ...
            if i%100==0:
                print('Iteration %d/%d, avg batch FPS %.2f '%(i, nruns, 1 / np.mean(timings)))

    print("Input shape:", input_data.size())
    # print("Output prediction size:", pred.size())
...
if __name__ == "__main__":
    ...
    # model = Yolov4().eval().to('cuda')

    n_classes = 20

    model = Yolov4(yolov4conv137weight=None,
									 n_classes=n_classes,
									 inference=True).eval().to('cuda')

    # print(model)

    # traced_model = torch.jit.trace(model, [torch.randn((1, 3, 320, 320)).to('cuda')])
    # trt_model = trtorch.compile(traced_model, {
        # "input_shapes": [(1, 3, 320, 320)],
        # "op_precision": torch.half,
        # "workspace_size": 3000000000,
    # })
# 
    # benchmark(trt_model, input_shape=(1, 3, 320, 320), nruns=1000)

    # benchmark(model, input_shape=(1, 3, 320, 320), nruns=1000)
