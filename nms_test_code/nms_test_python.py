...
def IOU(box1, box2):
    ...
    width = max(right_bottom_x - left_top_x, 0)
    height = max(right_bottom_y - left_top_y, 0)
...
def my_nms_cpu(boxes, scores, threshold):
    ...
    ## arg_sort
    sorted_boxes = [(n, scores[n]) for n in range(Num_Boxes)]
    
    sorted_boxes = sorted(sorted_boxes,
                          key=lambda bbox: bbox[1],
                          reverse=True)

    keep_index = [sorted_boxes[n][0] for n in range(Num_Boxes)] # (N, ) // sorted_index

    meta_index = keep_index.copy()
    
    while len(meta_index) > 0:
        idx_self = meta_index[0]
        for idx_other in meta_index[1:]:
            if IOU(origin_boxes[idx_self], origin_boxes[idx_other]) > threshold:
                meta_index.remove(idx_other)
                keep_index.remove(idx_other)
        meta_index.remove(idx_self)
...
test_data = []
counts_of_boxes = 16 * 16 * 16 * 16
image_size = (3200, 3200)
...
np.array(test_data, dtype=np.float32).tofile("nms_test_bboxes_big.bin")
np.array(test_data_scores, dtype=np.float32).tofile("nms_test_scores_big.bin")
(torch_out.detach().numpy().astype(dtype=np.np.intc)).tofile("nms_test_out_big.bin")

# np.array(test_data, dtype=np.float32).tofile("nms_test_bboxes_small.bin")
# np.array(test_data_scores, dtype=np.float32).tofile("nms_test_scores_small.bin")
# (torch_out.detach().numpy().astype(dtype=np.intc)).tofile("nms_test_out_small.bin")
